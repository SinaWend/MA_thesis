...
def plot_combined(out_dir, numbers, new_structure=False):
    ...
    domain_to_int = {domain: idx for idx, domain in enumerate(unique_domains)}
    domains_int = np.array([domain_to_int[domain] for domain in domains])

    # Define point size
    point_size = 10  # Adjust this value as needed
    ...
    plt.subplot(1, 2, 1)
    scatter = plt.scatter(umap_results[:, 0], umap_results[:, 1], c=labels, cmap='tab10', alpha=0.5, s=point_size)
    ...
    plt.subplot(1, 2, 2)
    scatter = plt.scatter(umap_results[:, 0], umap_results[:, 1], c=domains_int, cmap='tab20', alpha=0.5, s=point_size)
    plt.colorbar(scatter)
    plt.title('UMAP visualization (Domains)')
    legend_elements = create_legend(scatter, range(len(unique_domains)), "Domains")
    ...
    plt.subplot(1, 2, 1)
    scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1], c=labels, cmap='tab10', alpha=0.5, s=point_size)
    plt.colorbar(scatter)
    ...
    plt.subplot(1, 2, 2)
    scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1], c=domains_int, cmap='tab20', alpha=0.5, s=point_size)
    plt.colorbar(scatter)
...
def load_and_plot(out_dir, dataset_type, numbers, new_structure=False):
    for number in numbers:
        ...
        if feature_filename and label_filename and domain_filename:
            ...
            # Plot UMAP
            umap_results = umap.UMAP(n_components=2).fit_transform(features)
            plt.figure(figsize=(8, 5))  # Smaller plot size
            scatter = plt.scatter(umap_results[:, 0], umap_results[:, 1], c=labels, cmap='tab10', alpha=0.5, s=point_size)
            plt.colorbar(scatter)
