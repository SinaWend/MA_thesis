...
def load_center_dict(biospecimen_path):
    ...
    for item in biospecimen_data:
        ...
        if not submitter_id:
            continue
        submitter_id = submitter_id[:-1]  # Remove the last two characters
...
def get_task(na=None):
    ...
    for disease in disease_types:
        ...
        patch_folder = os.path.join(base_path, disease, 'patch', '0512_px_20_mag_otsu')
        ...
        for patch_file in patch_files:
            if patch_file.endswith(".h5"):
                ...
                submitter_id = patch_file[:11]  # Extract only the first 10 characters
                print(f"Patch file: {patch_file}")  # Debug print for the entire filename
                print(f"Extracted Submitter ID: {submitter_id}")  # Debug print for the extracted ID

                domain_name = center_dicts[disease].get(submitter_id, "Unknown")
                print(f"Domain name from center dict: {domain_name}")  # Debug print for domain name from center dict

                if domain_name in allowed_centers:
                    data.append({'path': patch_path, 'label': diagnosis_to_label[disease]})
...
def split_dataset(data, train_split, val_split):
    import pandas as pd
